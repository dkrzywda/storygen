...
- FR-016: An administrator can block an account and later unblock it. A blocked account cannot use the product and is told that its access has been suspended, rather than being shown a generic failure. The account overview shows which accounts are blocked. Priority: must-have
  > Scope boundary, **decided 2026-09-09** (this note previously read "is not decided here"): a blocked account is **refused at sign-in**. It never reaches a session, so it never sees the product — only the message that its access has been suspended. Two reasons recorded: the identity layer already carries its own notion of a suspended account and refuses authentication for one, so the product borrows an existing state instead of inventing a parallel one; and refusal at the door shows the least of the product to someone who has lost access. Consequence accepted knowingly: a blocked account also loses sight of its own saved generations, which are not deleted and return on unblocking.
- FR-017: An administrator can delete an account. Before the deletion is carried out they are told what will be destroyed with it — that account's saved generations — because the action cannot be undone. Priority: must-have
  > Scope boundary, **decided 2026-09-09**: deletion has **no recoverable stage of its own**. Blocking (FR-016) is the reversible way to put an account beyond use; deletion is the irreversible one. The product therefore carries two account states and not three, and each capability does exactly one thing.
...
- FR-018: An administrator can grant the administrator role to another account and revoke it, including on their own account, and including the last remaining administrator role. Before the last one is revoked they are told that administration will no longer be reachable through the product. Priority: must-have
  > Scope boundary, **decided 2026-09-09** (this note previously read that neither self-action nor the floor was decided): both are allowed. An administrator may act on their own account, and there is **no floor** — the last administrator role can be revoked.
  > Consequence recorded, and it is a one-way door: at the time this was decided the product had exactly one account, which held the role. Revoking it therefore ends administration, and nothing inside the product can restore it — the role is account data, and no unprivileged screen may write it, which is the property FR-015 rests on. Recovery exists but lives outside the product, in the data store's own console; that is where the role was first granted on 2026-09-08, so the path is known rather than hypothetical.
  > The warning in the requirement text is **added by the author of this amendment, not asked for**, and is recorded as such: the decision was that an administrator *may* revoke the last role, which says nothing about doing it unannounced. Warning first costs one sentence on screen and is the same principle the `## Success Criteria` guardrail already applies to irreversible destruction. If it is unwanted, it should be struck deliberately rather than left ambiguous.
...
7. ~~**What does "blocked" mean at the product's boundary?**~~ **RESOLVED 2026-09-09: refused at sign-in.** A blocked account never reaches a session and never sees the product; it is told its access has been suspended. Grounds: the identity layer already refuses authentication for a suspended account, so the product reuses that state rather than inventing a parallel one, and refusal at the door exposes the least to someone who has lost access. Accepted consequence: a blocked account also loses sight of its own generations, which survive and return on unblocking. No longer blocks FR-016.
8. ~~**Is deletion the only removal, or is there a recoverable state before it?**~~ **RESOLVED 2026-09-09: blocking is that state; deletion has no soft stage.** The product carries two account states, not three — FR-016 is the reversible removal, FR-017 the irreversible one — and the two capabilities stay separate rather than collapsing. The recorded consequence is unchanged: deleting an account destroys its saved generations and releases its share of the FR-013 ceiling, so after a deletion the ceiling no longer records what the application actually spent that day. No longer blocks FR-017.
9. ~~**May an administrator act on their own account, and can the last administrator role be revoked?**~~ **RESOLVED 2026-09-09: yes to both. No floor.** An administrator may act on their own account and may revoke the last remaining administrator role. The consequence was put to the author before the decision was recorded and accepted: the product had exactly one account holding the role, so exercising this ends administration, and no screen inside the product can restore it. Recovery is out-of-band, through the data store's own console — the same path by which the role was first granted on 2026-09-08. FR-018 additionally requires warning before the last revocation; that warning was **not** part of the decision and is flagged in FR-018 as the amendment author's own addition. No longer blocks FR-018.
