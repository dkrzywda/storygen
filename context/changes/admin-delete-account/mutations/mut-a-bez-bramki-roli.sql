-- MUTACJA A — obie bramki roli wolajacego zdjete (przed blokada i po niej).
-- Zmierzone: czerwieni przypadki 1-4 (odmowa dla nie-admina i dla zablokowanego admina).
--
-- JAK URUCHOMIC (sklej z testem, w jednej sesji):
--
--   CID=$(docker ps --filter name=supabase_db --format "{{.ID}}")
--   cat context/changes/admin-delete-account/mutations/mut-a-bez-bramki-roli.sql \
