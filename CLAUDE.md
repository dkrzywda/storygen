...
- **The LLM lives behind one module and there is no API key.** Cloudflare Workers AI, model `@cf/meta/llama-3.3-70b-instruct-fp8-fast`, reached through a Workers **binding** declared in `wrangler.jsonc` — there is no key, and there must never be one: that is what makes the credentials guardrail structural. `@src/lib/llm.ts` is the only file that touches `env.AI` or names the model; everything else passes a system prompt, a user prompt and a token ceiling. That isolation is the _condition_ of the open-weights portability argued in @context/foundation/tech-stack.md — break it and the escape hatch to another host is theoretical. Reach the binding via `import { env } from "cloudflare:workers"`; adapter v13 removed `Astro.locals.runtime` and reading it returns `undefined` at runtime, not a type error.
- **Call the model with `messages`, never a bare `prompt`.** Measured 2026-09-04: `env.AI.run(MODEL, { prompt })` bypasses the instruct model's chat template and produces degenerate repetition loops — 103–121 words against a 60-word ask, in 4 of 5 samples. The same request as `messages` (system + user) with an explicit `max_tokens` gave 18–38 words in 5 of 5. Always cap `max_tokens`; without it the model runs on.
- **`npx wrangler types` regenerates `worker-configuration.d.ts`** (committed, ESLint-ignored). Without it `cloudflare:workers` has no types and `astro check` fails. Rerun after every `wrangler.jsonc` change.
- **Every `supabase` command that touches the remote DB hangs for minutes instead of reporting a blocked port.** Measured 2026-09-08: `db push`, `db push --dry-run`, `db query` and `migration list --linked` all ran the full timeout with no output, then printed `Connect to your database by setting the env var correctly: SUPABASE_DB_PASSWORD` — which is misleading, because stdin was closed and the real cause was eight TCP retries against `aws-1-eu-west-1.pooler.supabase.com:5432`. The symptom reads as a password prompt. Confirm the port before debugging credentials: `timeout 15 bash -c "echo > /dev/tcp/aws-1-eu-west-1.pooler.supabase.com/5432"`. Postgres has no path over 443, so on a network that filters non-standard ports no CLI works — apply migrations through the SQL Editor and book the versions into `supabase_migrations.schema_migrations` by hand, or the next `db push` re-applies them.
- **`wrangler tail` never shows traffic for a preview version.** Measured 2026-09-08: three attempts against a version uploaded with `wrangler versions upload`, including a 10-minute background tail during live requests, produced zero lines — not even the connection banner. `--version-id` does not make it work. Debug a preview through the response body and the database, not the log.
